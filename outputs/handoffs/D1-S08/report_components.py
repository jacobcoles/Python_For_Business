"""Northbridge verified support components, v0.1-draft.
Released by the instructor at this session start; not a completed exercise.
No oracle data. Run from the course root; never modify shared inputs.
Fictional teaching thresholds, not financial policy.
"""
...
TRANSACTION_COLUMNS = ["transaction_id", "period", "entity", "account_code",
...
MOVEMENT_AMOUNT_THRESHOLD_CENTS = 100_00

MOVEMENT_PCT_THRESHOLD = Decimal("0.20")

CHECKS = {
    "C01": ("completeness_files", "critical", True),
    "C02": ("completeness_rows", "critical", True),
    "C03": ("duplicate_business_key", "critical", True),
    "C04": ("mapping_cardinality", "critical", True),
    "C05": ("unmapped_accounts", "critical", True),
    "C06": ("required_values", "critical", True),
    "C07": ("currency_supported", "critical", True),
    "C08": ("reconciliation_posted_amount", "critical", True),
    "C09": ("movement_review", "review", True),
}

SUMMARY_COLUMNS = ["check_id", "check_name", "severity", "scope", "expected", "observed",
                   "tolerance", "status", "affected_count", "detail_ref"]

EXCEPTION_COLUMNS = ["check_id", "severity", "source_file", "source_row", "entity", "period",
                     "transaction_id", "issue", "observed_value", "expected_value", "action_note"]

TRANSACTION_FILENAME = re.compile(r"transactions_(\d{4}-\d{2})_(N\d{2})\.csv")

PLAIN_NUMBER = r"-?\d+(\.\d+)?"

def to_cents(value):
    """Convert an amount in EUR to whole cents (a sum of 2-decimal amounts is a whole number of cents)."""
    return int((Decimal(repr(float(value))) * 100).quantize(Decimal("1"), rounding=ROUND_HALF_UP))

def money(value):
    """Round an amount once, half-up, to 2 decimals for presentation (M-09)."""
    return float(Decimal(repr(float(value))).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)) + 0.0

def money_text(value, signed=False):
    """Format an amount as text with 2 decimals, e.g. '29862.50' or '+1500.00'."""
...
def parse_amount(text_values):
    """Turn exported amount text into numbers; blank or non-numeric text becomes NaN, never 0."""
...
def is_blank(text_values):
    """True where an exported text value is empty or only spaces."""
    return text_values.astype(str).str.strip() == ""

def previous_period(period):
    """The calendar month before an ISO 'YYYY-MM' period."""
    year, month = int(period[:4]), int(period[5:7])
    return f"{year - 1}-12" if month == 1 else f"{year}-{month - 1:02d}"

def read_transaction_file(path):
    """Read one unit export as text (so '0400' stays '0400') and attach where each row came from."""
    ...
    rows["source_file"] = path.name                    # file name only, never a full path
    rows["source_row"] = range(1, len(rows) + 1)       # 1-based data row, header excluded
    return rows

def load_transaction_files(input_folder, relative_names, period):
    """Read the listed exports for one period. Files that are not there are reported, not guessed."""
    frames, missing = [], []
    for relative_name in relative_names:
        path = input_folder / relative_name
        match = TRANSACTION_FILENAME.fullmatch(path.name)
        if not match or match.group(1) != period:
            raise ValueError(f"Input manifest lists {relative_name!r}, which is not a "
                             f"transactions_{period}_<unit>.csv export for period {period}")
        if path.exists():
...
def read_reference_table(input_folder, manifest, file_name, override_path=None):
    """Read one reference table named in the manifest (as text). Returns (table or None, name used)."""
...
def load_inputs(input_folder, period, manifest_path=None, mapping_file=None):
    """Load exactly the files the input manifest names for this period -- never a folder glob.

    The transactions folder deliberately holds other periods too; loading "everything in the
    folder" would inflate every total. Every transaction row gets source_file and source_row.
    mapping_file optionally points at a corrected account mapping (e.g. F10's late delivery)
    so that raw exports never have to be edited.
    """
    ...
    if manifest.get("reporting_period") != period:
        raise ValueError(f"{manifest_path.name} is for {manifest.get('reporting_period')!r}, "
                         f"not the requested period {period!r}")

    transactions, missing_files = load_transaction_files(
        input_folder, manifest["transaction_files"], period)
    ...
    # Prior period, for the movement review (C09): either a released actuals table, or the
    # prior period's own exports, depending on what the manifest supplies.
    ...
    if manifest.get("prior_period_actuals"):
        ...
        prior_transactions, prior_missing_files = load_transaction_files(
            input_folder, manifest["prior_period_transaction_files"], prior_period)
    ...
    return {
        ...
        "received_files": [name for name in listed_files if name not in missing_files],
        ...
        "prior_missing_files": prior_missing_files,
        "prior_source": prior_source,
    }

def summary_row(check_id, scope, expected, observed, tolerance, status, affected_count, has_detail):
    """One control_summary.csv row, in the column order controls.md defines."""
    name, severity, _ = CHECKS[check_id]
    return {"check_id": check_id, "check_name": name, "severity": severity, "scope": scope,
            "expected": expected, "observed": observed, "tolerance": tolerance, "status": status,
            "affected_count": int(affected_count),
            "detail_ref": f"exceptions.csv check_id={check_id}" if has_detail else ""}

def exception_row(check_id, issue, observed_value, expected_value, action_note, source_file="",
                  source_row="", entity="", period="", transaction_id=""):
    """One exceptions.csv row, in the column order controls.md defines."""
    return {"check_id": check_id, "severity": CHECKS[check_id][1], "source_file": source_file,
            "source_row": source_row, "entity": entity, "period": period,
            "transaction_id": transaction_id, "issue": issue, "observed_value": observed_value,
            "expected_value": expected_value, "action_note": action_note}

def exception_from_source_row(check_id, row, issue, observed_value, expected_value, action_note):
    """An exception located by the source row it came from (works even when the key is blank)."""
    return exception_row(check_id, issue, observed_value, expected_value, action_note,
                         source_file=row["source_file"], source_row=int(row["source_row"]),
                         entity=row["entity"], period=row["period"],
                         transaction_id=row["transaction_id"])

def combine_scope_statuses(statuses):
    """Roll per-unit results into one check status: any fail, else any not_run, else pass."""
    if "fail" in statuses:
        return "fail"
...
def expected_exports(inputs):
    """The (entity, filename) pairs this period should deliver: expected_files.csv, else the manifest."""
...
def check_c01_completeness_files(inputs):
    """C01: every export the units are expected to send for this period was actually received."""
    period = inputs["period"]
    if inputs["expected_files"] is None:
        return summary_row("C01", period, "expected_files.csv available", "expected_files.csv missing or unreadable",
                           "n/a", "not_run", 0, False), []
    expected = expected_exports(inputs)
    if not expected:
        return summary_row("C01", period, f"expected files listed for {period}",
                           f"expected_files.csv lists no files for {period}", "n/a", "not_run", 0, False), []
    missing = [(entity, name) for entity, name in expected if name not in inputs["received_files"]]
    exceptions = [exception_row("C01", "expected export not received", "file not present in the input",
                                name, f"Obtain {name} from unit {entity}; do not report {period} without it.",
                                source_file=name, entity=entity, period=period)
                  for entity, name in missing]
    observed = f"{len(expected) - len(missing)} of {len(expected)} present"
    if missing:
        observed += "; missing: " + ", ".join(name for _, name in missing)
    return summary_row("C01", period, f"{len(expected)} expected files present", observed, "n/a",
                       "fail" if missing else "pass", len(missing), bool(exceptions)), exceptions

def control_total_row(inputs, entity):
    """The unit's own control-total row for this period from source_controls.csv, or None."""
...
def check_c02_completeness_rows(inputs):
    """C02: per unit, raw and posted row counts equal the counts the unit says it sent."""
    ...
    for entity, filename in expected_exports(inputs):
        ...
        posted_count = int((unit_rows["status"] == "posted").sum())    # exactly 'posted' only
        ...
        if raw_count == expected_raw and posted_count == expected_posted:
            ...
            exceptions.append(exception_row(
                "C02", "row count differs from the unit's control total",
                f"raw {raw_count}; posted {posted_count}", f"raw {expected_raw}; posted {expected_posted}",
                "Find the extra or missing rows before using this unit's figures.",
                source_file=filename, entity=entity, period=period))
    status = combine_scope_statuses(statuses) if statuses else "not_run"
    return summary_row("C02", period, "; ".join(expected_parts) or "control rows available",
                       "; ".join(observed_parts) or "no expected exports", "exact",
                       status, failing, bool(exceptions)), exceptions

def check_c03_duplicate_business_key(inputs):
    """C03: no business key (entity, period, transaction_id) appears twice, cancelled rows included."""
    period, rows = inputs["period"], inputs["transactions"]
    # Rows with a blank key part are C06's defect; counting them here would report it twice.
    keyed = rows[~(is_blank(rows["entity"]) | is_blank(rows["period"]) | is_blank(rows["transaction_id"]))]
    duplicates = keyed[keyed.duplicated(subset=BUSINESS_KEY, keep=False)]
    duplicated_keys = duplicates[BUSINESS_KEY].drop_duplicates()
    exceptions = []
    for _, row in duplicates.iterrows():
        key = f"{row['entity']}|{row['period']}|{row['transaction_id']}"
        exceptions.append(exception_from_source_row(
            "C03", row, "duplicate business key (entity, period, transaction_id)", key,
            "each key appears once",
            "Ask the unit which occurrence is genuine; never drop duplicates to make counts match."))
    observed = f"{len(duplicated_keys)} duplicated key(s), {len(duplicates)} row(s)"
    return summary_row("C03", period, "0 duplicated keys", observed, "exact",
                       "fail" if len(duplicated_keys) else "pass", len(duplicated_keys),
                       bool(exceptions)), exceptions

def check_c04_mapping_cardinality(inputs):
    """C04: the account mapping has exactly one row per account code (checked before any join)."""
    period, mapping = inputs["period"], inputs["account_mapping"]
    if mapping is None:
        return summary_row("C04", period, "account_mapping.csv available", "account mapping missing",
                           "n/a", "not_run", 0, False), []
    ...
    for position, row in mapping.iterrows():
        if row["account_code"] in offending_codes:
            exceptions.append(exception_row(
                "C04", "account code mapped more than once",
                f"{row['account_code']} -> {row['report_line']} ({row['account_name']})",
                "exactly one mapping row per account_code",
                "Ask finance which mapping is correct; do not join until the mapping is one-to-one.",
                source_file=inputs["mapping_name"], source_row=position + 1))
    observed = f"{len(offending_codes)} code(s) with more than one row"
    if offending_codes:
        observed += ": " + ", ".join(offending_codes)
    return summary_row("C04", period, "1 row per account_code", observed, "exact",
                       "fail" if offending_codes else "pass", len(offending_codes),
                       bool(exceptions)), exceptions

def check_c05_unmapped_accounts(inputs):
    """C05: every posted row's account code exists in the mapping. Unmapped rows are listed, never dropped."""
    period, rows, mapping = inputs["period"], inputs["transactions"], inputs["account_mapping"]
    if mapping is None:
        return summary_row("C05", period, "account_mapping.csv available", "account mapping missing",
                           "n/a", "not_run", 0, False), []
    known_codes = set(mapping["account_code"])
    posted = rows[rows["status"] == "posted"]
    # A blank code is C06's defect; it is reported there, once.
    unmapped = posted[~is_blank(posted["account_code"]) & ~posted["account_code"].isin(known_codes)]
    exceptions = [exception_from_source_row(
        "C05", row, "posted row uses an account code that is not in the mapping",
        f"account_code={row['account_code']}; amount={row['amount']}",
        f"an account_code listed in {inputs['mapping_name']}",
        "Ask finance for the mapping of this code; do not edit the export, bucket or drop the amount.")
    ...
    return summary_row("C05", period, "0 unmapped posted rows", observed, "exact",
                       "fail" if len(unmapped) else "pass", len(unmapped), bool(exceptions)), exceptions

def required_value_problems(row):
    """List what is wrong with one exported row's required values (empty list when nothing)."""
    problems = []
...
def check_c06_required_values(inputs):
    """C06: every row has its required values; a blank amount is not zero and a blank status is not 'posted'."""
    period, rows = inputs["period"], inputs["transactions"]
    exceptions = []
    for _, row in rows.iterrows():
        ...
        if problems:
            exceptions.append(exception_from_source_row(
                "C06", row, "required value missing or invalid: " + "; ".join(problems),
                "; ".join(f"{c}={row[c]!r}" for c in TRANSACTION_COLUMNS),
                "all seven values present; amount a number; status posted or cancelled; "
                "period and entity match the file name",
                "Return the row to the unit for correction; never default a blank to 0 or to posted."))
    return summary_row("C06", period, "0 rows with missing or invalid required values",
                       f"{len(exceptions)} row(s) with missing or invalid required values", "exact",
                       "fail" if exceptions else "pass", len(exceptions), bool(exceptions)), exceptions

def check_c07_currency_supported(inputs):
    """C07: every row is in EUR. Nothing is ever converted -- there is no rate in this pack."""
    period, rows = inputs["period"], inputs["transactions"]
    # A blank currency is C06's defect; it is reported there, once.
    wrong = rows[~is_blank(rows["currency"]) & (rows["currency"] != "EUR")]
    exceptions = [exception_from_source_row(
        "C07", row, "unsupported currency", f"currency={row['currency']}; amount={row['amount']}", "EUR",
        f"Return the row to unit {row['entity']}; no conversion is attempted.")
        for _, row in wrong.iterrows()]
    return summary_row("C07", period, "all rows EUR", f"{len(wrong)} non-EUR row(s)", "exact",
                       "fail" if len(wrong) else "pass", len(wrong), bool(exceptions)), exceptions

def check_c08_reconciliation(inputs):
    """C08: per unit, the posted amount received (as exported, before any filtering) equals the control total."""
    period, rows = inputs["period"], inputs["transactions"]
    statuses, expected_parts, observed_parts, exceptions, failing = [], [], [], [], 0
    for entity, filename in expected_exports(inputs):
        control = control_total_row(inputs, entity)
        ...
        # A total that cannot be formed is not_run, never a sum that treats blank as 0: that is a
        # blank/unparseable amount, or a blank/unknown status (the row cannot be classed as posted or not).
        ...
        if unusable_amounts or unusable_statuses:
            statuses.append("not_run")
            observed_parts.append(f"{entity}: not_run - {unusable_amounts} unusable amount(s) and "
                                  f"{unusable_statuses} unusable status(es), total cannot be formed - see C06")
            continue
        ...
        observed_parts.append(f"{entity}: {money_text(received_total)} "
                              f"(difference {money_text(difference_cents / 100, signed=True)})")
        if abs(difference_cents) <= RECONCILIATION_TOLERANCE_CENTS:
            statuses.append("pass")
        else:
            statuses.append("fail")
            failing += 1
            exceptions.append(exception_row(
                "C08", f"posted amount differs from the unit's control total by "
                       f"{money_text(difference_cents / 100, signed=True)}",
                money_text(received_total), money_text(expected_total),
                "Find the rows behind the difference with the unit before using its figures.",
                source_file=filename, entity=entity, period=period))
    status = combine_scope_statuses(statuses) if statuses else "not_run"
    return summary_row("C08", period, "; ".join(expected_parts) or "control rows available",
                       "; ".join(observed_parts) or "no expected exports", "EUR 0.01",
                       status, failing, bool(exceptions)), exceptions

def prior_actuals_for_review(inputs, entities):
    """Prior-period actuals for C09, or (None, reason) when they cannot be formed reliably."""
    ...
    unusable = (~prior_rows["status"].isin(ALLOWED_STATUSES)).sum() \
        + parse_amount(posted["amount"]).isna().sum() \
        + (posted["currency"] != "EUR").sum() \
        + (~posted["account_code"].isin(known_codes)).sum()
    if unusable:
...
def review_movements(current_actuals, prior_actuals, entities):
    """Compare each (unit, report line) with the prior period and say whether it needs a human look.

    Warn only when abs(movement) > EUR 100.00 AND abs(movement %) > 20 % (both strict).
    When prior is 0 the percentage is 'not_comparable' and a movement above EUR 100.00 is
    'new activity'. A line with no prior value at all cannot be reviewed ('not_run').
    """
    results = []
    for entity in entities:
        for line in REPORT_LINES:
            ...
            result = {"entity": entity, "report_line": line, "current": None, "prior": None,
                      "movement": None, "movement_pct": None, "outcome": "not_run"}
...
def pct_text(pct):
    """Show a movement percentage like '+24.33%', or 'not_comparable'."""
...
def check_c09_movement_review(inputs, earlier_rows):
    """C09: flag large period-on-period movements for review -- only on figures C01..C08 have passed (DEC-23)."""
    period = inputs["period"]
    expected_text = "abs(movement) <= 100.00 or abs(movement_pct) <= 20% (strict >)"
    tolerance = "EUR 100.00 and 20%"
    if any(row["status"] != "pass" for row in earlier_rows):
        return summary_row("C09", period, expected_text,
                           "not_run - critical controls did not all pass; movement not reviewed on unreliable figures",
                           tolerance, "not_run", 0, False), []
    ...
    for r in results:
        ...
        if r["outcome"] == "new_activity":
            ...
            observed_value = (f"{money_text(r['prior'])} -> {money_text(r['current'])}; movement "
                              f"{money_text(r['movement'], signed=True)}; movement_pct {pct_text(r['movement_pct'])}")
        observed_parts.append(f"{label} WARNING {observed_value}")
        exceptions.append(exception_row(
            "C09", issue, observed_value, expected_text,
            "Review with the unit and explain the movement in the handover note; "
            "do not alter data to silence a warning.",
            source_file=filename, entity=r["entity"], period=period))
    ...
    return summary_row("C09", period, expected_text, observed, tolerance, status, warnings,
                       bool(exceptions)), exceptions

def decide_release(control_summary_rows):
    """DEC-19: a critical fail or a required not_run blocks; else any warning; else ready."""
...
def run_controls(inputs):
    """Run C01..C09 before believing any total. Returns (control_summary, exceptions, release_status)."""
    summary_rows, exceptions = [], []
    for check in (check_c01_completeness_files, check_c02_completeness_rows,
                  check_c03_duplicate_business_key, check_c04_mapping_cardinality,
                  check_c05_unmapped_accounts, check_c06_required_values,
                  check_c07_currency_supported, check_c08_reconciliation):
...
def build_actuals(transactions, account_mapping, period, entities):
    """Posted, mapped, signed actuals: one row per (period, entity, report_line), no total row.

    Refuses to run on anything controls should have stopped: a non-EUR posted row, an unmapped
    posted row, a blank amount, or a mapping that is not one-to-one. Nothing is silently dropped.
    """
    posted = transactions[transactions["status"] == "posted"].copy()     # cancelled rows stay in the file only
    ...
    # validate='many_to_one' makes pandas refuse a duplicated mapping instead of multiplying rows (C04).
    ...
    for entity in entities:
        for line in REPORT_LINES:
            # A unit with no rows on a line gets a documented 0.00 -- only ever used after C01/C02.
            rows.append({"period": period, "entity": entity, "report_line": line,
                         "actual_amount": float(totals.get((entity, line), 0.0))})
...
def build_summary(actuals, budget, entities_table, period):
    """Add budget and favourable variance by line, then per-unit figures and a presentation total.

    favourable_variance = actual - budget on signed values, so positive is favourable on every
    line (M-06). Returns {'by_line', 'units', 'total'}; the total is a presentation object only
    and is never part of the unit table (M-08).
    """
    budget = budget.copy()
    ...
    for entity, lines in by_line.groupby("entity", sort=False):
        ...
        operating_result = actual["revenue"] + actual["direct_cost"] + actual["overhead"]          # M-04
        budget_operating_result = planned["revenue"] + planned["direct_cost"] + planned["overhead"]
        unit_rows.append({
            "period": period, "entity": entity, "entity_name": names.get(entity, ""),
            "revenue": actual["revenue"], "direct_cost": actual["direct_cost"], "overhead": actual["overhead"],
            "direct_cost_expense": -1 * actual["direct_cost"],                                        # M-05 display
            "overhead_expense": -1 * actual["overhead"],
            "operating_result": operating_result,
            "budget_operating_result": budget_operating_result,
            "favourable_variance": lines["favourable_variance"].sum(),
        })
    units = pd.DataFrame(unit_rows)

    money_columns = ["revenue", "direct_cost", "overhead", "direct_cost_expense", "overhead_expense",
                     "operating_result", "budget_operating_result", "favourable_variance"]
    total = {column: units[column].sum() for column in money_columns}       # sums each unit value once
...
def workbook_filename(period, run_number, release_status):
    """Northbridge_Management_Pack_<PERIOD>_r<NN>.xlsx, with _BLOCKED added when the release is blocked."""
...
def clear_rows(ws, first_row, last_column):
    """Empty every cell from first_row down (values and styles), so no earlier run's rows survive."""
...
def read_definitions_block(ws):
    """Copy the ByLine definitions text (and its fonts) wherever it currently sits on the sheet."""
    start = None
...
def fill_byline_sheet(ws, by_line):
    """ByLine: clear the data region, write signed rows from A2 (or a blocked note), then the definitions."""
    ...
    if by_line is None:
        ...
        for _, line in by_line.iterrows():
            values = [line["period"], line["entity"], line["entity_name"], line["report_line"],
                      line["report_line_label"], money(line["actual_amount"]),
                      money(line["budget_amount"]), money(line["favourable_variance"])]
            for column, value in enumerate(values, start=1):
                cell = ws.cell(next_row, column, value)
                if column >= 6:
                    cell.number_format = MONEY_FORMAT
            next_row += 1
    next_row += 1                                        # exactly one blank row before the definitions
    for text, font in definitions:
        ws.cell(next_row, 1, text).font = font
        next_row += 1

def fill_controls_sheet(ws, control_summary):
    """Controls: clear the data region and write one row per check, status colour-coded."""
    ...
    for offset, (_, check) in enumerate(control_summary.iterrows()):
        ...
        status_cell.value = check["status"]              # not_run is written as text, never left empty
        status_cell.fill = PatternFill("solid", fgColor=STATUS_FILLS[check["status"]])

def fill_exceptions_sheet(ws, exceptions):
    """Exceptions: clear the data region and write the detail, or 'No exceptions raised.'."""
...
def fill_summary_sheet(ws, period, release_status, generated_at, pack_version, summary):
    """Summary: header cells, then unit rows and Total -- figures only when the release is not blocked."""
    ws["C3"], ws["C4"], ws["C5"], ws["C6"] = period, release_status, generated_at, pack_version
    for row_number in range(9, 13):                       # clear B9:I11 and D12:I12 (B12 'Total' is template)
    ...
    if summary is None:
        ...
        return
    columns = ["revenue", "direct_cost_expense", "overhead_expense", "operating_result",
               "budget_operating_result", "favourable_variance"]
...
def readme_lines(inputs, release_status, control_summary, generated_at, pack_version, output_name):
    """The handover note: what went in, what came out, why it was (or was not) released, how to rerun."""
    not_passed = control_summary[control_summary["status"] != "pass"]
    lines = [
        f"Reporting period: {inputs['period']} (prior period for movement review: {inputs['prior_period']})",
        f"Generated (UTC): {generated_at}",
        f"Case-pack version: {pack_version}",
        f"Output file: {output_name}",
        "",
        "Inputs consumed (file names only):",
        f"  input manifest: {inputs['manifest_name']}",
        "  transaction exports: " + (", ".join(inputs["received_files"]) or "none"),
    ]
    if inputs["missing_files"]:
        lines.append("  listed but NOT received: " + ", ".join(inputs["missing_files"]))
    lines += [
        f"  account mapping: {inputs['mapping_name']}",
        "  reference tables: entities.csv, budget.csv, source_controls.csv, expected_files.csv",
        f"  prior period: {inputs['prior_source'] or 'none supplied'}",
        "",
    ...
    if not_passed.empty:
        ...
        for _, check in not_passed.iterrows():
            lines.append(f"  {check['check_id']} {check['check_name']} = {check['status']} "
                         f"(affected {check['affected_count']}): {check['observed']}")
    if release_status == "blocked":
        lines += ["What must happen next: resolve every failed or not-run control above with the unit or",
                  "finance (see the Exceptions sheet for the source file and row), then rerun. Do not edit",
                  "raw exports and do not circulate figures from this run. No figures are shown in this pack."]
    elif release_status == "ready_with_warnings":
        lines += ["What must happen next: explain each warning on the Exceptions sheet in the handover note.",
                  "A warning is a request to look, not an instruction to change data. It stays visible."]
    lines += [
        "",
        "Assumptions:",
        "  Signs: revenue positive, costs negative; operating result = revenue + direct cost + overhead.",
        "  Summary shows Direct cost and Overhead as positive expense magnitudes; ByLine is signed.",
        "  Favourable variance = actual - budget on signed values; positive is favourable.",
        "  Cancelled rows are excluded from totals and kept in the source files. No currency conversion.",
        "  Reconciliation tolerance EUR 0.01; movement review warns when abs(movement) > EUR 100.00 and",
        "  abs(movement %) > 20%. These are fictional teaching thresholds, not financial policy.",
        "  Summary!C14 is a formula; open in Excel and verify recalculation (pending target review).",
        "",
        "To rerun: use the documented entry point from the course root, with the same input/period.",
        "  Instructor reference entry point is recorded in its module docstring and session notes.",
        "  Learner component handouts are not standalone programs: record your completed calling",
        "  script/notebook, configuration, output folder and explicit overwrite choice in the handover.",
        "",
        "All figures are synthetic. This pack is not an audit opinion.",
    ]
    return lines

def fill_readme_sheet(ws, lines):
    """ReadMe: keep the title in A1, replace everything below with the handover note."""
    clear_rows(ws, first_row=2, last_column=1)
    for offset, text in enumerate(lines):
        if text:
            ws.cell(3 + offset, 1, text)

def write_workbook(output_folder, inputs, release_status, control_summary, exceptions, summary,
                   run_number=1, overwrite=False, template_path=TEMPLATE_PATH, generated_at=None,
                   pack_version=None):
    """Open the master template, fill it and save it as a NEW dated file. The master is never written.

    Control gate: when release_status is 'blocked', Summary and ByLine carry no figures, Controls and
    Exceptions are fully written, and the file name ends in _BLOCKED.
    """
    ...
    if release_status == "blocked":
        summary = None                                   # the gate: no figures in a blocked pack
    ...
    fill_readme_sheet(wb["ReadMe"], readme_lines(inputs, release_status, control_summary,
                                                 generated_at, pack_version, output_path.name))
    # W4-26: output-only legibility/print layout. Protected widths, header styles, formula
    # and the master file stay unchanged. Default portrait pagination split Summary columns.
    ws = wb['Summary']
...
def write_csv(table, path):
    """Write a table as UTF-8 CSV with '\\n' line endings and no index (schemas.md conventions)."""
